...
export function registerEdgePath(id: string, el: SVGPathElement) {
  const length = measure(el)
  paths.set(id, { el, length, planned: length })
  geometry.value++
}

export function unregisterEdgePath(id: string) {
  paths.delete(id)
}
 ...
 */
export function invalidateEdgePath(id: string) {
...
export function registerToken(key: string, el: SVGGElement) {
  elements.set(key, el)
...
export function unregisterToken(key: string) {
  elements.delete(key)
}
...
export function installFlowRuntime() {
  if (installed) return
  installed = true
...
export function stopFlowRuntime() {
